...
We denote the null hypothesis $H_{0}$: $\theta(t)=0$ and the alternate hypothesis $H_1$: $\theta(t)=\theta_1(t)$ for $t> 0$ where $t$ represents the information fraction for a study.
...
As before, we use a shorthand notation with $\theta$ representing $\theta()$, $\theta=0$ to represent
$\theta(t)\equiv 0$ for all $t$ and $\theta_1$ to represent $\theta_1(t_k)$, the alternate hypothesis effect size at analysis $k$, $1\leq k\leq K$.
