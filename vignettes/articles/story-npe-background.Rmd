---
title: "NPES: Non-Proportional Effect Size in Group Sequential Design"
subtitle: "Distribution Assumptions and Notation"
...
While it is motivated primarily by a use for when designing a time-to-event trial under non-proportional hazards (NPH), we have simplified and generalized the concept here. The model is used initially for logrank and weighted logrank tests that will be considered initially by @tsiatis1982repeated.
...
## The continuous model, S-process and E-process
...
We assume for a given integer $N>0$ that $X_{i}$ are independent, $i=1,2,\ldots, N$.
...
For $1\le k\le K$, we define the S-process as 
$$S_k=\sum_{i=1}^{n_k}X_i.$$
...
For the Cox model, we would define $S_k=\mathcal{I}_k\hat\theta_k$ where $\mathcal{I}_k$ is the statistical information at analysis $k$.
We see that $t_k$, $1\leq k\leq K$ is the so-called information fraction at analysis $k$ in that
...
It can be useful to think of $B_k$ as a sum of independent random variables divided by the final analysis statistical information.
The S-process can also be thought of like a Brownian motion process.

## Summary of E-, S-, Z- and B-processes

```{r, message=FALSE, warning=FALSE, echo=FALSE}
mt <- tibble::tribble(
  ~Statistic, ~Example, ~"Expected value", ~Variance,
  "$\\hat\\theta_k$", "$\\bar X_k$", "$\\theta(t_k)$", "$\\mathcal{I}_k^{-1}$",
  "$S_k=\\mathcal{I}_k\\theta_k$", "$n_k\\bar X_k$", "$\\mathcal{I}_k\\theta(t_k)$", "$\\mathcal{I}_k$",
