---
title: "Canonical joint distribution of Z-score and B-values under null and alternate hypothesis"
...
$m_{i,1}$, the $O_i$ has a Bernoulli distribution with parameter $E_i$. So the null conditional mean and variance of $\Delta_i$ are 0 and $V_i = E_i(1 - E_i)$, respectively. 
We let $r$ denote the randomization ratio (experimental:control) and $d_k$ be the total number of events at analysis $k$.
Unconditionally, $\Delta_i$ are mean 0 random variables with variance $E(V_i)=r/(r+1)^2$ under the null hypothesis; we note that when $r=1$ we have $E(V_i)=1/4.$
Conditioning on $d_k$, we have $\text{Var}(\sum_{i=1}^{d_k} \Delta_i | H_0)) = E(\sum_{i=1}^{d_k} V_i)=d_kr/(r+1)^2.$
...
Given the index $K$ for the final analysis, the ratio of the statistical information at analysis $k$ to statistical information at analysis $K$ under the null hypothesis is called **information fraction**:
...
In the rest of this vignette, we will discuss the value of $\mathcal I_{k, H_0}, \mathcal I_{k, H_1}$. To derive the statistical information, we introduce the B-values:
...
As depicted in Section 2.1.3 of @proschan2006statistical, the stochastic process formulation presented in @harrington1984procedures and @tsiatis1982group provides insights that sequence of B-values, denoted as ${B_1, \ldots, B_K}$ follows a multivariate normal distribution and behaves asymptotically like Brownian motion.
...
Under the alternate hypothesis, for 2 B-values ($B_i, B_j$ with $i \leq j$), the distribution of $\{B_k\}_{k = 1, \ldots, K}$ has the following structure:
